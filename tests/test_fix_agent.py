...
class TestSubmitFixProposalTool:
    def test_first_submission_populates_holder(self):
        ...
        result = tool.invoke(
            {
                "edits": [{"file": "A.java", "start_line": 1, "end_line": 1, "new_content": "x\n", "reason": "r"}],
                "summary": "fixed it",
                "status": "verified",
            }
        )
    ...
    def test_second_submission_ignored(self):
        ...
        result = tool.invoke({"edits": [], "summary": "second", "status": "draft"})
...
class TestBuildFixProposal:
    def test_empty_holder_returns_draft(self):
        proposal = _build_fix_proposal({}, "pid", "did", "2026-01-01T00:00:00+00:00")
    ...
    def test_verified_holder_builds_correct_proposal(self):
        ...
        proposal = _build_fix_proposal(holder, "pid", "did", "2026-01-01T00:00:00+00:00")
...
class TestBuildUserMessage:
    def test_with_conclusion_includes_fix_direction(self):
        report = _make_report(fix_direction="Add null guard before name.length()")
        msg = _build_user_message(report, "pid-123", "/src/project")
    ...
    def test_without_conclusion_uses_fallback_text(self):
        ...
        msg = _build_user_message(report, "pid-456", "/src/project")
...
class TestFixAgentStructure:
    ...
    def test_four_tools_registered(self, tmp_path):
        ...
        with (
            ...
            patch("src.agent.fix_agent.create_react_agent", side_effect=fake_create_react_agent),
        ):
    ...
    def test_agent_exception_returns_draft_proposal(self, tmp_path):
        ...
        with (
            patch("src.agent.fix_agent.ChatOpenAI"),
            patch("src.agent.fix_agent.create_react_agent", side_effect=crashing_agent),
        ):
            proposal = agent.run(_make_report())
    ...
    def test_exception_after_submit_uses_submitted_result(self, tmp_path):
        config = _make_config(tmp_path)
        ws_root = tmp_path / "ws"
        agent = FixAgent(config=config, workspace_root=str(ws_root))
        ...
        with (
            patch("src.agent.fix_agent.ChatOpenAI"),
            patch("src.agent.fix_agent.create_react_agent", side_effect=agent_submits_then_crashes),
        ):
            proposal = agent.run(_make_report())
